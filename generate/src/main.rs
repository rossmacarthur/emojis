...
fn main() -> Result<()> {
    ...
    // start with extra variations
    for seq in &unicode_data.variations {
        let var = format!("{seq}\u{fe0f}");
        match unicode_data.emojis.iter().enumerate().find(|(_, e)| {
            e.as_str() == seq || e.as_str() == var || e.as_str() == format!("{var}\u{20e3}")
        }) {
            Some((i, _)) => {
                unicode_map.insert(var, i.to_string());
            }
            None => {
                bail!("warning: variation '{}' not found in unicode data", seq);
            }
        }
    }
...
fn write_emojis_slice<W: io::Write>(
    ...
    for (i, emoji) in unicode_data.emojis.iter().enumerate() {
        ...
        write_emoji_struct(
            ...
            unicode_data.emojis[default_skin_tone_index].skin_tones,
        )?;
        writeln!(w, ",")?;
